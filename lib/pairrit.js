...
const addUserToPair = (userName, pairName, channelId) => {
  ...
  const dataBasePair = getPair(pairKey, pairsCollection);
...
const noPairsMessage = () => {
  const lines = [
    'No pairs are currently active in this channel.',
    'Start one now using `/pair join`'
  ];
...
const listsPairsInChannel = (channelId, res) => {
  ...
  pairsCollection.findDoc({}, (err, pairs) => {
    p('pairs: ', pairs);
  });

  pairsCollection.findDoc({}, (err, activePairs) => {
