...
const db               = new massive.connectSync({connectionString: connectionString});
const pairsCollection  = db.pairs;
...
const addUserToPair = (userName, pairName, channelId) => {
  return new Promise((resolve, reject) => {
    ...
    getPair(pairKey, pairsCollection)
      .then((dataBasePair) => {
        const currentPair     = _.assign(immutableValues, dataBasePair)
        const newParticipants = new Set([...currentPair.participants, userName]);
        const newPair         = _.extend(_.omit(["id"])(currentPair), {participants: [...newParticipants]});

        return setPair(newPair, pairsCollection);
      })
      .then(() => {
        resolve(true);
      });
...
const listsPairsInChannel = (channelId, res) => {
  getPairs(channelId, pairsCollection)
    .then((activePairs) => {
      if(_.isEmpty(activePairs)) {
        ...
        _.uniqBy('hash')(activePairs).forEach((pair) => {
